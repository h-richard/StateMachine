...
public class StateMachineValidator extends AbstractStateMachineValidator {
	...
	public void checkOperation(Xmod_Operation op) {
		...
		for (String p : params)
			checkChain(p, root, StateMachinePackage.Literals.XMOD_OPERATION__PARAMETERS_TAG, params.indexOf(p));

		// checks the object chain
		// it follows the 'on' keyword, stating from which class the method belongs to
		checkChain(op.getObjectTag(), root, StateMachinePackage.Literals.XMOD_OPERATION__OBJECT_TAG, -1);

		// checks the optional return chain
		// it follows the 'returns' keyword
		if (op.getReturnTag() != null)
			checkChain(op.getReturnTag(), root, StateMachinePackage.Literals.XMOD_OPERATION__RETURN_TAG, -1);
	...
	private static List<EClass> getModelClasses(Predicate<EClass> filter) {
		EPackage pack = StateMachinePackage.eINSTANCE;
